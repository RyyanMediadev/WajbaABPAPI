...
using Wajba.Models.PopularItemsDomain;
using Wajba.Models.CartsDomain;
using Wajba.Models.Orders;
using Wajba.Enums;
using Wajba.Dtos.CartContract;
using Wajba.CartService;
using Wajba.CompanyService;
...
public class CartController : WajbaController
{
    ...
    public CartController(CartAppService CartAppService,
        ItemAppServices ItemAppServices, PopularItemAppservice PopularItemAppservice, OrderSetupAppService orderSetupAppService
    ...
    public async Task<IActionResult> AddCartItem(CartItemDto cartItemDto)
    {
        ...
        if (!ModelState.IsValid || cartItemDto.Quantity < 1)
        {
        ...
        //{
        //    string Id = " 6b29fc40-ca47-1067-b31d-00dd010662da";
        //};
        ...
        if (existingCart == null)
        {
            ...
            //  await _unitOfWork.AddAsync(existingCart);
        ...
        CartItem existingItem = existingCart.CartItems.FirstOrDefault(i => i.ItemId == cartItemDto.ItemId);
        ItemDto item = await _ItemAppServices.GetByIdAsync(cartItemDto.ItemId);
        PopularItem popularItem = await _PopularItemAppservice.GetpopItemByitemId(item.Id);
        //PopularItem popularItem1 = await _PopularItemAppservice.GetPopitem(cartItemDto.ItemId, 7);
        if (existingItem != null)
        {
            existingItem.Quantity += cartItemDto.Quantity;
            existingItem.Notes = cartItemDto.Notes;
            existingItem.price = item.Price;
            existingItem.ImgUrl = item.imageUrl;
        }
        else
        ...
        ReturnCartItemDto returnCartItemDto = new ReturnCartItemDto()
        {
            Variations = existingItem.SelectedVariations.Select(v => new ReturnCartItemVariationDto()
            {
            }).ToList(),
            Addons = existingItem.SelectedAddons.Select(a => new ReturnCartItemAddonDto()
            {
            }).ToList(),
            Extras = existingItem.SelectedExtras.Select(e => new ReturnExtraDto()
            {
            }).ToList()
        };
        return Ok(new { success = true, message = "item added to cart successfully", data = returnCartItemDto });
    }
